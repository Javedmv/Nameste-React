...
const restList = [
  ...
  },
  {
    ...
    "cta": {
      "link": "https://www.swiggy.com/restaurants/kfc-mg-road-kacheripady-kochi-53437",
      "type": "WEBLINK"
    }
  },
  {
...
const RestCart = ({name,cloudinaryImageId,cuisines,avgRating}) => {
  return (
    <div className="card">
      <img src={"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,h_600/"+cloudinaryImageId} alt="food" ></img>
      <h2>{name}</h2>
      <h3>{cuisines.join(", ")}</h3>
...
const Body = ()=>{
  return (
    <div className="restuarentList">
      {restList.map(res => <RestCart {...res.info} key={res.info.id} />)}
    </div>
  )
}
